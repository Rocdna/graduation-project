...
// 使用 ref 创建对 canvas 的引用
const canvasRef = ref(null);

// 在组件挂载时初始化 WebGL 和流体模拟
onMounted(() => {
  const canvas = canvasRef.value;
  if (!canvas) return;

  // 以下是原始 WebGL 和流体模拟的逻辑，基本保持不变
  ...
  // 以下是其他工具类、着色器、渲染逻辑等（与 React 版本一致）
  // 为简洁起见，这里省略，直接复用 React 版本中的代码
  // 包括 Material、Program、createProgram、compileShader 等
  class Material {
  ...
  // createFBO、createDoubleFBO、resizeFBO 等函数保持不变
  ...
  // calcDeltaTime、resizeCanvas、updateColors 等函数保持不变
...
// 监听 props 变化，更新 config（相当于 React 的 useEffect 依赖数组）
watch(
  () => [

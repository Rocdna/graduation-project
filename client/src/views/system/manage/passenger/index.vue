...
import { fetchGetPassengerList, fetchDeletePassenger, fetchBatchDeletePassengers } from '@/service/api';
...
defineOptions({ name: 'PassengerManage' });
...
const {
  ...
  columns: () => [
    ...
    { prop: '_id', label: $t('page.manage.user.id'), width: 70, showOverflowTooltip: true },
    { prop: 'username', label: $t('page.manage.user.nickName'), width: 120, showOverflowTooltip: true },
    {
      prop: 'profile.gender',
    ...
    { prop: 'phone', label: $t('page.manage.user.userPhone'), minWidth: 120 },
    {
      ...
      align: 'center',
      formatter: row => {
        const status = row.profile?.status;
        if (!status) return '';
        const tagMap: Record<string, UI.ThemeColor> = {
          online: 'success',
          offline: 'danger',
    ...
    {
      prop: 'profile.defaultPaymentMethod',
      label: $t('page.manage.user.defaultPaymentMethod'),
      minWidth: 70, // 增加宽度
      formatter: row => row.profile?.defaultPaymentMethod || '-'
    },
    {
      prop: 'operate',
      width: 120, // 操作列固定宽度
...
async function handleBatchDelete() {
  try {
    ...
    const { data, error } = await fetchBatchDeletePassengers(checkedRowKeys.value);
    if (!error) {
...
async function handleDelete(id: string) {
  try {
    const { error } = await fetchDeletePassenger(id);
    if (!error) {
